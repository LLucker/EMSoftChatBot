...
public class ChatBot extends TelegramLongPollingBot {
    ...
    public void onUpdateReceived(Update update) {
        if (update.hasMessage()) {
            ...
            if (message.hasText()) {
                ...
                MYFRAME.setMensagem(">> "+message.getChat().getFirstName() + "("+message.getChat().getId()+"):" + mensagem + "\n");
                ...
                else if(mensagem.contains("/lor")){
                 sendMessageRequest.setChatId("235209097");
                 sendMessageRequest.setText(mensagem.substring(4, mensagem.length()));
                 System.out.println(mensagem.substring(4, mensagem.length()));
                 enviarMensagem(sendMessageRequest);   
    ...
    public void enviarMensagem(SendMessage sendMessageRequest) {
        try {
            ...
            MYFRAME.setMensagem("<< "+sendMessageRequest.getText());
        } catch (TelegramApiException e) {
            //
